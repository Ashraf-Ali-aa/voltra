...
import { InteractiveCounterWidget } from '~/widgets/InteractiveCounterWidget'

/**
 * Screen demonstrating the InteractiveCounterWidget with the useInteractiveWidget hook.
 *
 * This screen showcases best practices for building interactive widgets:
 ...
 * 4. Display action logs for debugging and verification
 */
export default function InteractiveCounterScreen() {
  ...
  const onAction = useCallback(
    async (action: WidgetActionEvent) => {
      addLog(`Action received: ${action.actionName}`)
      ...
      const payload = renderAndroidWidgetToJson([
        {
          ...
          content: <InteractiveCounterWidget count={newCount} lastUpdated={new Date().toLocaleTimeString()} />,
        },
      ])

      addLog(`Widget updated with count: ${newCount}`)
      return payload
    },
    [count, addLog]
  )
  ...
  const { updateWidget } = useInteractiveWidget('interactive_counter', isListening ? { onAction } : undefined)
  ...
  const handleManualUpdate = useCallback(async () => {
    ...
    const payload = renderAndroidWidgetToJson([
      {
        size: { width: 250, height: 280 },
        content: <InteractiveCounterWidget count={count} lastUpdated={new Date().toLocaleTimeString()} />,
      },
    ])
  ...
  // Determine count display color
  const countColor = count > 0 ? '#22C55E' : count < 0 ? '#EF4444' : '#FFFFFF'

  return (
    <View style={styles.container}>
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.content}>
        <Text style={styles.heading}>Interactive Counter Widget</Text>
        <Text style={styles.subheading}>
          A polished example showcasing interactive widget features. The widget responds to button presses and updates in
          real-time.
        </Text>
        ...
        <Card>
          ...
          <View style={styles.instructionsList}>
            <Text style={styles.instructionItem}>1. Pin the &quot;Interactive Counter&quot; widget to your home screen</Text>
